...
uses
  ...
  GraphUnit, StdCtrls, Buttons, EC_Buf, EC_Str, ComCtrls, CheckLst;
...
TState = class(TGraphPoint)
  public
    FMove:integer; // 0-None 1-Move 2-Follow 3-Jump 4-Landing 5-Free
...
begin
  ...
  FOnTalk:=bd.GetWideStr;
  FOnActCode:=bd.GetWideStr;
...
begin
  ...
  li:=TList.Create();
  FindAllPoint('TState',li);
  for i:=0 to li.Count-1 do begin
    if(li.Items[i] = self) then Result:='State #' + IntToStr(i) + #13#10;
  end;
  li.Clear();
  li.Free();
  ...
  end else if FMove=5 then begin
    Result:=Result+'Move : Free'+#13#10;
  end;
  ...
  if (FOnTalk<>'') or (FOnActCode<>'') or (FEMsg<>'') then begin
    Result:=Result+'~';

    if Length(FOnTalk)<=40 then begin
      Result:=Result+FOnTalk+#13#10;
    end else begin
      Result:=Result+Copy(FOnTalk,1,40)+' ...'+#13#10;
    end;

    Result:=Result+'~';

    if Length(FOnActCode)<=40 then begin
      Result:=Result+FOnActCode+#13#10;
    end else begin
      Result:=Result+Copy(FOnActCode,1,40)+' ...'+#13#10;
    end;
...
begin
  ...
  for i:=0 to FState.FAttack.Count-1 do
  begin
  ...
  begin
    ...
    if gp is TGroup then
    begin
      if FState.FAttack.IndexOf(gp)<0 then
      begin
        CheckListAttack.Items.Add(gp.FText);
        CheckListAttack.Items.Objects[CheckListAttack.Items.Count-1]:=gp;
      end;
...
begin
  ...
  FindAllPoint('TState',li);
  for i:=0 to li.Count-1 do begin
    if(li.Items[i] = FState) then Caption:=Caption+' #' + IntToStr(i);
  end;
  li.Clear();
  li.Free();
  ...
  MemoMsgOut.Text:=FState.FOnTalk;
  MemoMsgIn.Text:=FState.FOnActCode;
...
begin
  ...
  for i:=0 to CheckListAttack.Items.Count-1 do
    if CheckListAttack.Checked[i] then
  ...
  FState.FOnTalk:=MemoMsgOut.Text;
  FState.FOnActCode:=MemoMsgIn.Text;
...
var
  ...
  str:WideString;
begin
    if CheckListAttack.Checked[CheckListAttack.ItemIndex] then
    begin
        ...
        i:=CheckListAttack.ItemIndex+1;
        while i<CheckListAttack.Items.Count do
        begin
