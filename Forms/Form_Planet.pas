...
const
  ar:array[0..7] of WideString = ('Maloc','Peleng','People','Fei','Gaal','Kling','None','PirateClan');
...
begin
  ...
  if CA(FEconomy,0) then
  begin
    tstr:='';
    for i:=0 to 2 do
    begin
      if CA(FEconomy,i+1) then
      begin
        if tstr<>'' then tstr:=tstr+',';
        tstr:=tstr+ae[i];
      end;
  ...
  if CA(FGoverment,0) then
  begin
    tstr:='';
    for i:=0 to 4 do
    begin
      if CA(FGoverment,i+1) then
      begin
        if tstr<>'' then tstr:=tstr+',';
        tstr:=tstr+ag[i];
      end;
  ...
  if (FRangeMin>0) or (FRangeMax<100) then
    sl.Add('Range : '+IntToStr(FRangeMin)+','+IntToStr(FRangeMax));


  if FDialog<>nil then sl.Add('Dialog : '+FDialog.FText);
...
begin
  ...
  CheckBoxOwnerPirate.Checked:=CA(FPlanet.FOwner,8);
  CheckBoxOwnerByPlayer.Checked:=CA(FPlanet.FOwner,9);
...
begin
  ...
  SCA(FPlanet.FOwner,8,CheckBoxOwnerPirate.Checked);
  SCA(FPlanet.FOwner,9,CheckBoxOwnerByPlayer.Checked);
