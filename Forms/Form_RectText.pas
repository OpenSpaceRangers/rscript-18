...
begin
    ...
    ImageR.Canvas.Font.Name:=FontComboBoxTFont.Text;
...
function EnumFontClBack(var lp: TEnumLogFontW;
...
begin
  ...
  tstr:=lp.elfLogFont.lfFaceName;

  with TFormRectText(lpData), FontComboBoxTFont do
  begin

    for i:=0 to Items.Count-1 do
      if Items[i] = tstr then exit;

    Items.Add(tstr);
  end;
end;

procedure TFormRectText.FormCreate(Sender: TObject);
var lpLogfont:tagLOGFONTW;
begin
  lpLogfont.lfCharSet := DEFAULT_CHARSET;
  //lpLogfont.lfFaceName:='';
  lpLogfont.lfFaceName[0]:=WideChar(0);
  lpLogfont.lfPitchAndFamily:=0;
  //lpLogfont.
  EnumFontFamiliesExW(Canvas.Handle,lpLogfont, @EnumFontClBack, Integer(Self), 0);
end;
