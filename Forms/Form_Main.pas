...
TFormMain = class(TForm)
    ...
    procedure SpeedButtonBuildClick(Sender: TObject);

    procedure LoadFromFile(fname:WideString);
private
...
uses Form_RectText,Global, Form_Build;
...
var
    ...
    i:integer;
begin
    for i:=0 to GImageList.Count-1 do
    begin
...
begin
    ...
    FCfg.LoadFromFile('rscriptCFG.txt');
    ...
    SetLength(tstr,1024);
    ZeroMemory(PAnsiChar(tstr),1024);
    if GetModuleFileNameA(0,PAnsiChar(tstr),1024)=0 then raise Exception.Create('GetModuleFileNameW');
...
begin
    ...
    cv.Pen.Color:=ColorBG;
    cv.Brush.Color:=ColorBG;
    cv.Brush.Style:=bsSolid;
    cv.Rectangle(0,0,DI.Width,DI.Height);

    for i:=0 to GGraphRectText.Count-1 do
    begin
        grt:=GGraphRectText.Items[i];

        grt.Draw(cv,GViewPos);

        if GCurGraphUnit=grt then
        begin
    ...
    begin
        ...
        BitBlt(cv.Handle, tr.Left, tr.Top, tr.Right-tr.Left, tr.Bottom-tr.Top, iu.Fbh, 0, 0, SRCCOPY);
    ...
    if GCurGraphUnit is TGraphLink then
    begin
        cv.Pen.Color:=ColorLinkActive;
        cv.Brush.Color:=ColorLinkActiveArrow;
        cv.Brush.Style:=bsSolid;
    ...
    if (FMode=mRectText) and (FBeginAction) and (not FSize) then
    begin
...
begin
    ...
    if (Button=mbRight) and (ssCtrl	in Shift) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphRectText) and (FMode=mRectText) and (not FBeginAction) and (not FSize) and (not FMove) then
    begin
        FMove:=true;
    ...
    begin
        if (GCurGraphUnit=nil) or (GCurGraphUnit is TGraphRectText) or ((GCurGraphUnit is TGraphPoint) and FNewLink) then
        begin
            FMoveWorld:=true;
            FMoveWorldLP:=Point(x,y);
            Screen.Cursor:=crSizeAll;
        ...
        else if (GCurGraphUnit is TGraphPoint) and (FMode=mNormal) then
        begin
            FMove:=true;
    ...
    else if (Button=mbLeft) and (FMode=mRectText) and (not FBeginAction) and (not FSize) and (not FMove) then
    begin
    ...
    else if (Button=mbLeft) and (FMode=mRectText) and (not FBeginAction) and (FSize) and (not FMove) then
    begin
        PanelDI.OnMouseMove:=DIMouseMove;
        PanelDI.OnMouseUp:=DIMouseUp;
    ...
    else if (Button=mbLeft) and (FMode=mNormal) and (not FMove) and (not FMoveWorld) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) then
    begin
    ...
    else if (FMode=mNewPoint) and (not FMoveWorld) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphLink) then
    begin
        if Application.MessageBox('Delete ?','Message',MB_YESNO or MB_ICONQUESTION)=ID_YES then begin
            with GCurGraphUnit as TGraphLink do
            begin
                gp:=FBegin;
                gp2:=FEnd;
            end;
    ...
    else if (FMode=mNewPoint) and (not FMoveWorld) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) then
    begin
        if Application.MessageBox('Delete ?','Message',MB_YESNO or MB_ICONQUESTION)=ID_YES then
        begin
            i:=0;
            while i<GGraphLink.Count do
            begin
                gl:=GGraphLink.Items[i];
                if (gl.FBegin=GCurGraphUnit) or (gl.FEnd=GCurGraphUnit) then
                begin
                    gl.Free;
                    GGraphLink.Delete(i);
                end else inc(i);
            ...
            begin
                gp:=GGraphPoint.Items[i];
                if gp.FParent=GCurGraphUnit then
                begin
                    gp.FPos:=gp.WorldPos;
                    gp.FParent:=nil;
                end;
    ...
    else if (FMode=mNewPoint) and (not FMoveWorld) and (not ((GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint))) then
    begin
        ...
        for i:=0 to GGraphPointInterface.Count-1 do
        begin
            if no<>TGraphPointInterface(GGraphPointInterface.Items[i]).FGroup then
            begin
...
begin
    ...
    if (FMoveWorld or FMove) and (Button=mbRight) then
    begin
        FMove:=false;
        FMoveWorld:=false;
        if (FMode=mRectText) and FBeginAction and (FSize) then
        begin
    ...
    else if (Button=mbLeft) and (FMode=mRectText) and FBeginAction and (not FSize) and (not FMove) then
    begin
        FBeginAction:=false;
        Mouse.Capture:=0;
        PanelDI.OnMouseMove:=nil;
        ...
        if (abs(FRectAction.Left-FRectAction.Right)>2) and ((abs(FRectAction.Top-FRectAction.Bottom)>2)) then
        begin

            GCurGraphUnit:=TGraphRectText.Create;
            with GCurGraphUnit as TGraphRectText do
            begin
            ...
            if FormRectText.ShowModal<>mrOk then
            begin
                GCurGraphUnit.Free;
    ...
    else if (Button=mbLeft) and (FMode=mRectText) and FBeginAction and (FSize) and (not FMove) then
    begin
        FBeginAction:=false;
        Mouse.Capture:=0;
        PanelDI.OnMouseMove:=nil;
    ...
    else if (Button=mbLeft) and (FMode=mNormal) and FNewLink then
    begin
        ...
        if (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) and (GCurGraphUnit<>FNewLinkPoint) then
        begin
            if FNewParent then
            begin
        ...
        else if (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) and (GCurGraphUnit=FNewLinkPoint) and FNewParent then
        begin
...
begin
    ...
    if FMoveWorld then
    begin
        if (x<>FMoveWorldLP.x) or (y<>FMoveWorldLP.y) then
        begin
    ...
    else if (FMode=mNormal) and FMove then
    begin
    ...
    else if (FMode=mRectText) and FMove then
    begin
        grt:=GCurGraphUnit as TGraphRectText;
        OffsetRect(grt.FRect,x-FMoveWorldLP.x,y-FMoveWorldLP.y);
        FMoveWorldLP:=Point(x,y);
        fd:=true;
    end else if (FMode=mNormal) and FNewLink then
    begin
        tr.Left:=min(FNewLinkPos.x-GViewPos.x,FNewLinkPoint.WorldPos(GViewPos).x);
        tr.Top:=min(FNewLinkPos.y-GViewPos.y,FNewLinkPoint.WorldPos(GViewPos).y);
        tr.Right:=max(FNewLinkPos.x-GViewPos.x,FNewLinkPoint.WorldPos(GViewPos).x)+1;
    ...
    if (not FMoveWorld) and (not FMove) and (FMode=mRectText) and (not FBeginAction) then
    begin
        cur:=nil;

        for i:=GGraphRectText.Count-1 downto 0 do
        begin
            grt:=GGraphRectText.Items[i];
            tr:=grt.FRect;
        ...
        begin
            ...
            begin
                grt:=GGraphRectText.Items[i];
                if ((x+GViewPos.x)>=grt.FRect.Left) and ((x+GViewPos.x)<grt.FRect.Right) and ((y+GViewPos.y)>=grt.FRect.Top) and ((y+GViewPos.y)<grt.FRect.Bottom) then
                begin
                    cur:=grt;
                    break;
                end;
    ...
    else if (not FMove) and (not FMoveWorld) and ((FMode=mNormal) or (FMode=mNewPoint)) then
    begin
        cur:=nil;
        ...
        begin
            gp:=GGraphPoint.Items[i];
            tr:=gp.GetRectImage(GViewPos);
            if (x>=tr.Left) and (x<tr.Right) and (y>=tr.Top) and (y<tr.Bottom) then
            begin
                cur:=gp;
                break;
            end;
        ...
        if (not FNewLink) and (cur=nil) then
        begin
            for i:=0 to GGraphLink.Count-1 do
            begin
                gl:=GGraphLink.Items[i];
                if gl.TestHit(Point(x,y)) then
                begin
                    cur:=gl;
                    break;
                end;
        ...
        if cur<>GCurGraphUnit then
        begin
            old:=GCurGraphUnit;
            GCurGraphUnit:=cur;

            if GCurGraphUnit=nil then
            begin
                PanelInfo.Visible:=false;
            end
            else if GCurGraphUnit is TGraphPoint then
            begin
                gp:=(GCurGraphUnit as TGraphPoint);
                ...
                if PanelInfo.Left<0 then
                begin
                    PanelInfo.Left:=0;
                end
                else if (PanelInfo.Left+PanelInfo.Width)>PanelDI.Width then
                begin
                    PanelInfo.Left:=PanelDI.Width-PanelInfo.Width;
                end;

                if (PanelInfo.Top+PanelInfo.Height)>PanelDI.Height then
                begin
                    PanelInfo.Top:=tp.y-ImageSize(gp.FImage).y-PanelInfo.Height;
                end;

            end
            else if GCurGraphUnit is TGraphLink then
            begin
                gl:=(GCurGraphUnit as TGraphLink);
            ...
            if old<>nil then
            begin
                if old is TGraphPoint then
                begin
                    tr:=(old as TGraphPoint).GetUpdateRect(GViewPos);
                    InvalidateRect(PanelDI.Handle,@tr,false);
                end
                else if old is TGraphLink then
                begin
            ...
            if GCurGraphUnit<>nil then
            begin
                if GCurGraphUnit is TGraphPoint then
                begin
...
begin
    ...
    if (FMode=mRectText) and (not FMoveWorld) and (not FMove) and (not FBeginAction) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphRectText) then
    begin
        FBeginAction:=false;
        if FormRectText.ShowModal=mrAbort then
        begin
    ...
    else if (FMode=mNormal) and (not FMoveWorld) and (not FMove) and (not FBeginAction) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) then
    begin
        if (GCurGraphUnit as TGraphPoint).DblClick then DIDraw;
    end
    else if (FMode=mNormal) and (not FMoveWorld) and (not FMove) and (not FBeginAction) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphLink) then
    begin
        if (GCurGraphUnit as TGraphLink).DblClick then DIDraw;
    end;
end;
...
begin
    ...
    begin
        ...
        begin
            if (FMode=mRectText) and (not FBeginAction) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphRectText) then
            begin
        ...
        else if Msg.wParam=VK_NEXT then
        begin
            if (FMode=mRectText) and (not FBeginAction) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphRectText) then
            begin
                no:=GGraphRectText.IndexOf(GCurGraphUnit);
                if no<(GGraphRectText.Count-1) then
                begin
                    GGraphRectText.Items[no]:=GGraphRectText.Items[no+1];
                    GGraphRectText.Items[no+1]:=GCurGraphUnit;
                    DIDraw;
        ...
        else if ((Msg.wParam=VK_INSERT) or (Msg.wParam=VK_SPACE)) and (FMode=mNormal) and (not FMove) and (not FMoveWorld) and (not FNewLink) and (not ((GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint))) then
        begin
            GetCursorPos(tp);
        ...
        else if (Msg.wParam=VK_DELETE) and ((FMode=mNormal) or (FMode=mNewPoint)) and (not FMove) and (not FMoveWorld) and (not FNewLink) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphLink) then
        begin
            if Application.MessageBox('Delete ?','Message',MB_YESNO or MB_ICONQUESTION)=ID_YES then
            begin
                with GCurGraphUnit as TGraphLink do
        ...
        else if (Msg.wParam=VK_DELETE) and ((FMode=mNormal) or (FMode=mNewPoint)) and (not FMove) and (not FMoveWorld) and (not FNewLink) and (GCurGraphUnit<>nil) and (GCurGraphUnit is TGraphPoint) then
        begin
            if Application.MessageBox('Delete ?','Message',MB_YESNO or MB_ICONQUESTION)=ID_YES then
            begin
                i:=0;
        ...
        else if (Msg.wParam=VK_CONTROL) and (FNewLink) and (not FNewParent) then
        begin
            FNewParent:=True;
            DIDraw;
        end;
    ...
    begin
        if (Msg.wParam=VK_CONTROL) and (FNewLink) and (FNewParent) then
        begin
            FNewParent:=false;
            DIDraw;
        end;
...
begin
  ...
  if FMoveWorld or FMove or FNewLink then
  begin
    FNewLink:=false;
    FMove:=false;
    FMoveWorld:=false;
  ...
  else if (FMode=mRectText) and (FBeginAction or FMove) then
  begin
    FMove:=false;
    FBeginAction:=false;
    Screen.Cursor:=crDefault;
...
begin
  if Application.MessageBox('New ?','Message',MB_YESNO or MB_ICONQUESTION)=ID_YES then
  begin
    AllClear;
    GViewPos:=Point(0,0);
    MainNew;
...
begin
  ...
  if (FCurFileName='') or ((GetAsyncKeyState(VK_CONTROL) and $8000)=$8000) then
  begin
  ...
  b.SaveInFile(PAnsiChar(AnsiString(FCurFileName)));
  b.Free;
...
begin
  ...
  b:=TBufEC.Create;
  b.LoadFromFile(PAnsiChar(AnsiString(FCurFileName)));
  b.BPointer:=0;
...
procedure TFormMain.LoadFromFile(fname:WideString);
var
  b:TBufEC;
  sdir:AnsiString;
begin
...
begin
  ...
  begin
    FormMain.Caption:=FProgZag+' 1.'+IntToStr(PVersion);
  end
  else begin
    FormMain.Caption:=FProgZag+' 1.'+IntToStr(PVersion)+' ['+FCurFileName+']';
  end;
end;


procedure TFormMain.SpeedButtonBuildClick(Sender: TObject);
begin
  BuildRun();
end;
