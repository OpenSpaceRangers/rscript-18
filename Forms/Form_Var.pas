...
begin
  ...
  FType:=bd.GetInteger();
  FInit:=bd.GetWideStr;
  if GFileVersion>=$00000003 then FGlobal:=bd.GetBoolean;
end;
...
begin
  ...
  else if FType=4 then Result:='float'
  else if FType=5 then Result:='array';
...
begin
  ...
  if ComboBoxType.ItemIndex = 5 then Label3.Caption:='Cnt elem. :'
  else Label3.Caption:='Init state :';

  Label3.Visible:=(ComboBoxType.ItemIndex <> 0);
  EditInitState.Visible:=(ComboBoxType.ItemIndex <> 0);
...
procedure TFormVar.ComboBoxTypeChange(Sender: TObject);
begin
  if ComboBoxType.ItemIndex = 5 then Label3.Caption:='Cnt/[init] :'
  else Label3.Caption:='Init state :';
