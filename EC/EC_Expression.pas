...
uses SysUtils,Classes,EC_Buf{,DebugMsg};

// 1001 - catch or finally in try

function HexToStrEXP(zn:Cardinal):WideString;
function BinToStrEXP(zn:Cardinal):WideString;
...
TVarEC=class(TObject)
    protected
        ...
        FVLibraryFun:array of Cardinal;
        FVFun:TCodeEC;
        FVClass:TCodeEC;
        FVArray:TVarArrayEC;
        FVRef:TVarEC;
    ...
    public
        ...
        procedure SetDW(zn:Cardinal);
        procedure SetFloat(zn:double);
        procedure SetStr(const zn:WideString);
        ...
        property VDW:Cardinal read GetDW write SetDW;
        ...
        procedure ArrayChange(cnt:integer; de:integer=0);
...
TVarArrayEC=class(TObject)
    ...
    protected
        function FindIndex(const tname:WideString):integer;
        function FindInsertIndex(const tname:WideString):integer;
    ...
    public
        ...
        function GetVar(const tname:WideString):TVarEC;
        function GetVarNE(const tname:WideString):TVarEC;

        procedure ChangeName(no:integer);

        procedure Del(no:integer); overload;
        procedure Del(el:TVarEC); overload;
        procedure Del(const tname:WideString); overload;

        procedure Add(el:TVarEC); overload;
        procedure Insert(tono:integer; el:TVarEC); overload;

        function Add(const tname:WideString; vtype:VariableTypeEC=vtUnknown):TVarEC; overload;
        function Insert(tono:integer; const tname:WideString; vtype:VariableTypeEC=vtUnknown):TVarEC; overload;
...
TCodeAnalyzerUnitEC = class(TObject)
    public
        ...
        function CErrorEXP(tsme:integer):WideString;
end;
...
TExpressionEC=class(TObject)
    ...
    public
        ...
        function Compiler(ca:TCodeAnalyzerEC; curclass:TCodeEC=nil; castart:TCodeAnalyzerUnitEC=nil; caend:TCodeAnalyzerUnitEC=nil; endcompl:PCardinal=nil):WideString;
...
TCodeProcessEC=class(TObject)
    ...
    public
        ...
        procedure ThrowAdd(tv:TVarEC); overload;
        procedure ThrowAddNotCreate(tv:TVarEC);
        procedure ThrowAdd(const tstr:WideString); overload;
...
TCodeEC=class(TObject)
    private
        ...
        FProcess:TCodeProcessEC;	// Init for external function
        FDebug:TCodeDebugEC;		// Init for external function
    public
        constructor Create;
        destructor Destroy; override;
        ...
        property Parent:TCodeEC read FParent;
        property Process:TCodeProcessEC read FProcess;
        property Debug:TCodeDebugEC read FDebug;
        ...
        function Compiler(ca:TCodeAnalyzerEC; codeunit:integer; funinclude:FunctionIncludeUnitEC=nil; el:TCodeAnalyzerUnitEC=nil; elendcomp:PCardinal=nil; curclass:TCodeEC=nil):WideString;
        function CompilerR(ca:TCodeAnalyzerEC; codeunit:integer; funinclude:FunctionIncludeUnitEC; el:TCodeAnalyzerUnitEC; insertto:TCodeUnitEC; elendcomp:PCardinal; elcontinue:PCardinal; opBreak:TCodeUnitEC; opContinue:TCodeUnitEC; curclass:TCodeEC):WideString;
        procedure Link(va:TVarArrayEC);
        procedure LinkAll(va:TVarArrayEC);
        procedure LinkClass;
        procedure UnLink(va:TVarArrayEC);
        ...
        procedure RunFunction(const name:WideString; cpr:TCodeProcessEC; cd:TCodeDebugEC=nil; vars:TVarArrayEC=nil);
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
function CompareStrEXP(s1,s2:PWideChar):integer; cdecl;
asm
		push    esi
		push    edi
		push    ebx
...
begin
    Result:='';
    while(zn<>0) do begin
        Result:=Chr((zn-(zn div 2)*2)+Cardinal('0'))+Result;
        zn:=zn div 2;
    end;
    if Result='' then Result:='0';
end;
...
function DecodeDWEXP(var elt:TCodeAnalyzerUnitEC; var zn:Cardinal):boolean;
var
    el:TCodeAnalyzerUnitEC;
    ch:WideChar;
    len,i:integer;
begin
    ...
    if (ch='h') or (ch='H') then begin
        for i:=0 to len-2 do begin
            ch:=el.FStr[i+1];
            if (ch>='0') and (ch<='9') then zn:=zn*16+(Cardinal(ch)-Cardinal('0'))
            else if (ch>='a') and (ch<='f') then zn:=zn*16+(10+Cardinal(ch)-Cardinal('a'))
            else if (ch>='A') and (ch<='F') then zn:=zn*16+(10+Cardinal(ch)-Cardinal('A'))
            else Exit;
        end;
        ...
        for i:=0 to len-2 do begin
            ch:=el.FStr[i+1];
            if (ch>='0') and (ch<='1') then zn:=zn*2+(Cardinal(ch)-Cardinal('0'))
            else Exit;
        end;
    end else Exit;
    el:=el.FNext;
...
function caeToStr(cae:caeTypeEC):widestring;
begin
    if cae=caeEnter then Result:=''//#13#10
		else if (cae=caeSpace) then Result:=' '
		else if (cae=caeOpen1) then Result:='('
		else if (cae=caeClose1) then Result:=')'
		else if (cae=caeOpen2) then Result:='{'
		else if (cae=caeClose2) then Result:='}'
		else if (cae=caeOpen3) then Result:='['
		else if (cae=caeClose3) then Result:=']'
		else if (cae=caeOpen4) then Result:='/*'
		else if (cae=caeClose4) then Result:='*/'
		else if (cae=caeCom) then Result:='//'
		else if (cae=caePoint) then Result:='.'
		else if (cae=caePointer) then Result:='->'
		else if (cae=caeAdd) then Result:='+'
		else if (cae=caeSub) then Result:='-'
		else if (cae=caeMul) then Result:='*'
		else if (cae=caeDiv) then Result:='/'
		else if (cae=caeMod) then Result:='%'
		else if (cae=caeBitAnd) then Result:='&'
		else if (cae=caeBitOr) then Result:='|'
		else if (cae=caeBitXor) then Result:='^'
		else if (cae=caeBitNot) then Result:='~'
		else if (cae=caeAnd) then Result:='&&'
		else if (cae=caeOr) then Result:='||'
		else if (cae=caeNot) then Result:='!'
		else if (cae=caeShl) then Result:='<<'
		else if (cae=caeShr) then Result:='>>'
		else if (cae=caeAssume) then Result:='='
		else if (cae=caeEqual) then Result:='=='
		else if (cae=caeNotEqual) then Result:='!='
		else if (cae=caeLess) then Result:='<'
		else if (cae=caeMore) then Result:='>'
		else if (cae=caeLessEqual) then Result:='<='
		else if (cae=caeMoreEqual) then Result:='>='
		else if (cae=caeSemicolon) then Result:=';'
		else if (cae=caeColon) then Result:=':'
		else if (cae=caeComma) then Result:=','
    else if (cae=caeStr) then Result:=''
		else if (cae=caeName) then Result:=''
    else Result:='';
end;
...
function TVarEC.GetDW:Cardinal;
begin
    if FVType=vtUnknown then Result:=0
    else if FVType=vtInt then Result:=FVInt
    else if FVType=vtDW then Result:=FVDW
...
procedure TVarEC.SetDW(zn:Cardinal);
begin
    if FVType=vtUnknown then begin VType:=vtInt; FVInt:=zn; end
    else if FVType=vtInt then FVInt:=zn
    else if FVType=vtDW then FVDW:=zn
...
procedure TVarEC.SetStr(const zn:WideString);
begin
...
begin
    ...
    if len>0 then begin
	    astr:=FVStr;
    	bufa:=Cardinal(PWideChar(FVStr));
	    for i:=0 to len-1+1 do begin PAnsiChar(bufa)^:=astr[i+1]; bufa:=bufa+1; end;
        if (len and 1)<>0 then begin
            SetLength(FVStr,(len shr 1)+1);
        end else SetLength(FVStr,(len shr 1));
    end;
end;

procedure TVarEC.ToUnicode;
var
	wstr:AnsiString;
    len:integer;
begin
	if FVType<>vtStr then begin
    	ChangeVType(vtStr);
        Exit;
    end;
    len:=Length(FVStr);
    if len>0 then begin
    	wstr:=PAnsiChar(PWideChar(FVStr));
        FVStr:=wstr;
    end;
end;
...
procedure ArrayChangeREC(va:TVarArrayEC; newcnt:integer);
var
	i,curcnt:integer;
    tvar:TVarEC;
    dem:array of integer;

	procedure ArrayCelcDimension(va:TVarArrayEC);
	begin
		SetLength(dem,High(dem)+1+1);
	    dem[High(dem)]:=va.Count;

    	if (va.Count>0) and (va.Items[0].VType=vtArray) then begin
	        ArrayCelcDimension(va.Items[0].VArray);
	    end;
	end;
begin
	curcnt:=va.Count;
    if newcnt=curcnt then Exit
    else if newcnt<curcnt then begin
	    for i:=newcnt to curcnt-1 do begin
        	tvar:=va.Items[i];
    	    if (tvar.FVType=vtArray) and (tvar.VArray<>nil) then ArrayClearREC(tvar.VArray);
	    end;
	    for i:=curcnt-1 downto newcnt do begin
        	va.Del(i);
        end;
    end else begin
    	dem:=nil;
        ArrayCelcDimension(va);
        ArrayInitREC(va,dem,0);
    end;
end;
...
procedure TVarEC.ArrayChange(cnt:integer; de:integer=0);
begin
    if (VType<>vtArray) then Exit;
    if cnt<=0 then begin ArrayClear; Exit; end
    else if de>0 then Exit
    else if VArray=nil then ArrayInit([cnt])
    else ArrayChangeREC(VArray,cnt);
end;
...
begin
    ...
    case s1.VType of
        ...
        vtDW:VDW:=Cardinal((s1.VDW<>0) and (s2.VDW<>0));
...
begin
    ...
    case s1.VType of
        ...
        vtDW:VDW:=Cardinal((s1.VDW<>0) or (s2.VDW<>0));
...
begin
    ...
    case s1.VType of
        ...
        vtDW:VDW:=Cardinal(s1.VDW=s2.VDW);
...
begin
    ...
    if(Items[i].FVType = vtArray) then Items[i].VArray.Clear();
        Items[i].Free;
    end;
    ClearNoFree;
end;
...
function TVarArrayEC.FindIndex(const tname:WideString):integer;
var
...
begin
    ...
    while true do begin
        ...
        el:=IndexToVar(icur);
        cz:=CompareStrEXP(PWideChar(tname),PWideChar(el.Name));
...
function TVarArrayEC.FindInsertIndex(const tname:WideString):integer;
var
    istart,iend,icur,cz:integer;
    el:TVarEC;
begin
...
//    pushad
    ...
    mov     ebx,el
    mov     eax,self
    mov     ecx,TVarArrayEC(eax).FCount
    xor     edx,edx
    mov     esi,TVarArrayEC(eax).FIndex
    mov     edi,TVarArrayEC(eax).FItem
    mov     eax,-1
//    mov     Result,eax
    test    ecx,ecx
    jz      @@l3
...
function TVarArrayEC.GetItem(no:integer):TVarEC; assembler;
asm
//        push    eax
        push    ebx
        mov     ebx,self
...
function TVarArrayEC.GetVar(const tname:WideString):TVarEC;
begin
    Result:=GetVarNE(tname);
    if Result=nil then raise ExceptionExpressionEC.Create('Var not found:'+tname);
end;

function TVarArrayEC.GetVarNE(const tname:WideString):TVarEC;
var
    istart,iend,icur,cz:integer;
    el:TVarEC;
begin
...
procedure TVarArrayEC.Del(const tname:WideString);
begin
    Del(FindIndex(tname));
end;
...
function TVarArrayEC.Add(const tname:WideString; vtype:VariableTypeEC):TVarEC;
var
    el:TVarEC;
begin
...
function TVarArrayEC.Insert(tono:integer; const tname:WideString; vtype:VariableTypeEC):TVarEC;
var
    el:TVarEC;
begin
    el:=TVarEC.Create(vtype);
...
//                        inc(FLast.FLen);
						inc(i);
zsme:=i+1; zlen:=0;
					end else if ((i+1)<count) and (wstr[i+1+1]='x') then begin
if (zsme>=0) and (zlen>0) then begin FLast.FStr:=FLast.FStr+Copy(wstr,zsme+1,zlen-1); inc(FLast.FLen,zlen-1); end;
                        FLast.FLen:=FLast.FLen+2;
						i:=i+2;
                        zn:=0; cnt:=0;
                        while (i<count) and (cnt<4) do begin
                        	sim:=wstr[i+1];
                        	if (sim>='0') and (sim<='9') then zn:=zn*16+integer(sim)-integer('0')
                            else if (sim>='a') and (sim<='f') then zn:=zn*16+integer(sim)-integer('a')+10
                            else if (sim>='A') and (sim<='F') then zn:=zn*16+integer(sim)-integer('A')+10
                            else break;
                            inc(cnt);
    	                    FLast.FLen:=FLast.FLen+1;
							inc(i);
                        end;
						FLast.FStr:=FLast.FStr+WideChar(zn);
						i:=i-1;
zsme:=i+1; zlen:=0;
					end;
...
begin
    ...
    if count<>count2 then begin
        Result:=FLast.CErrorEXP(FLast.FSme+FLast.FLen); Exit;
        Exit;
    end;
    ...
    try
		...
		if i>0 then begin
            Result:=FLast.CErrorEXP(FLast.FSme+FLast.FLen);
            av:=nil;
            Exit;
        end;
    except
...
function TCodeAnalyzerUnitEC.CErrorEXP(tsme:integer):WideString;
var el:TCodeAnalyzerUnitEC;
    len:integer;
    tstr,tstr2:Widestring;

    procedure checkSme(el:TCodeAnalyzerUnitEC; var str:WideString);
    begin
      if tsme=el.FSme then str:='!'+str
      else if tsme=(el.FSme+el.FLen) then str:=str+'!'
      else if (tsme>el.FSme) and (tsme<(el.FSme+el.FLen)) then str:='!'+str+'!';
    end;
begin
    tstr:=caeToStr(FType);
    if tstr='' then tstr:=FStr;
    checkSme(self,tstr);

    len:=0;
    el:=FPrev;
    while (el<>nil) and (len<20) do
    begin
      tstr2:=caeToStr(el.FType);
      if tstr2='' then tstr2:=el.FStr;
      checkSme(el,tstr2);

      tstr:=tstr2+tstr;

      len:=len+length(tstr2);
      el:=el.FPrev;
    end;
    ...
    Result:='sme='+IntToStr(tsme)+#13#10+tstr;
end;

function TCompilerUnitEC.CErrorEXP(tsme:integer):WideString;
var el:TCompilerUnitEC;
    len:integer;
    tstr,tstr2:Widestring;
    
    procedure checkSme(el:TCompilerUnitEC; var str:WideString);
    begin
      if tsme=el.FSme then str:='!'+str
      else if tsme=(el.FSme+el.FLen) then str:=str+'!'
      else if (tsme>el.FSme) and (tsme<(el.FSme+el.FLen)) then str:='!'+str+'!';
...
{$WARNINGS OFF}
function TExpressionEC.Compiler(ca:TCodeAnalyzerEC; curclass:TCodeEC; castart:TCodeAnalyzerUnitEC; caend:TCodeAnalyzerUnitEC; endcompl:PCardinal):WideString;
var
    el,el2:TCodeAnalyzerUnitEC;
    cp:TCompilerEC;
...
begin
    ...
    if endcompl<>nil then endcompl^:=Cardinal(el);
    ...
    if fopen<>0 then begin
        ...
        Result:=el.CErrorEXP(el.FSme+el.FLen);
        cp.Free;
        Exit;
    end;
    ...
    while cpu<>nil do begin
        if cpu.FType=cutConstInt then begin
            ...
            if not exvar.BuildPath then begin Result:=cpu.CErrorEXP(cpu.FSme); cp.Free; Exit; end;
    ...
    while cp.FFirst.FNext<>nil do begin
        ...
        if cpu=nil then begin
            ...
            if cp.FFirst=nil then begin
                ...
                Result:=cp.FFirst.CErrorEXP(cp.FFirst.FSme);
            end;
            cp.Free;
            Exit;
    ...
    while i<FInstrCount do begin
        ...
        if (eli.FType=eieMinus) or (eli.FType=eieBitNot) or (eli.FType=eieNot) then begin
            ...
            if (exvar1var.VType=vtLibraryFun) then begin
                ...
                for u:=(eli.FCount-2-1) downto 0 do begin
                    ...
                    case tvar.FVLibraryFun[2+u] of
                        ...
                        3: begin
                            ...
                            if Length(tvar2.FVStr)<=0 then dwzn:=0
                            else dwzn:=Cardinal(PWideChar(tvar2.FVStr));
                        end;
                    else
                        dwzn:=0;
                    end;
                ...
                exvar0.FVar.SetVType(vtUnknown);
                for u:=2 to eli.FCount-1 do begin
...
procedure TCodeProcessEC.ThrowAdd(const tstr:WideString);
var
    pth:PCodeProcessThrowEC;
begin
    pth:=HeapAlloc(GetProcessHeap(),0,sizeof(TCodeProcessThrowEC));
    pth.FVar:=TVarEC.Create(vtStr);
    pth.FVar.VStr:=tstr;
    FThrow.Add(pth);
end;
...
begin
    ...
    cudes:=FFirst;
    while cu<>nil do begin
        if cu.FVar<>nil then begin
        	cudes.FVar:=FLocalVar.GetVarNE(cu.FVar.FName);
        end;
        cu:=cu.FNext;
        cudes:=cudes.FNext;
    end;
...
function TCodeEC.Compiler(ca:TCodeAnalyzerEC; codeunit:integer; funinclude:FunctionIncludeUnitEC; el:TCodeAnalyzerUnitEC; elendcomp:PCardinal; curclass:TCodeEC):WideString;
begin
//    Clear;
    Result:='';
...
{$WARNINGS OFF}
function TCodeEC.CompilerR(ca:TCodeAnalyzerEC; codeunit:integer; funinclude:FunctionIncludeUnitEC; el:TCodeAnalyzerUnitEC; insertto:TCodeUnitEC; elendcomp:PCardinal; elcontinue:PCardinal; opBreak:TCodeUnitEC; opContinue:TCodeUnitEC; curclass:TCodeEC):WideString;
var
    ...
    begin
        ...
        while (el.FType=caeName) and IsVarEXP(el.FStr) do begin
            if FLocalVar.GetVarNE(el.FStr)<>nil then begin Result:=el.CErrorEXP(el.FSme); Exit; End;
            AddVarByType(oper,el.FStr);

            if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; End;
    ...
    while el<>nil do begin
        if el.FType=caeName then begin
            ...
            if InDeclareVar(el) then begin
                ...
                if el.FType<>caeSemicolon then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                el:=el.FNext;

                if (elcontinue<>nil) and (countopen=0) then begin
                    elcontinue^:=Cardinal(el);
                    Exit;
                end;
                ...
                while true do begin
                    if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                    if (el.FNext.FType<>caeOpen1) or (el.FNext.FNext=nil) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                    ...
                    if el2=nil then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                    if el2.FType<>caeClose1 then begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                    el:=el2.FNext;
                ...
                if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                if (el.FNext.FType<>caeOpen1) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                if (el.FNext.FNext=nil) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme+el.FNext.FLen); Exit; end;
                ...
                if el.FType=caeSemicolon then begin
                    ...
                    while true do begin
                        ...
                        if el2.FType=caeSemicolon then begin
                            ...
                            el:=el2.FNext;
                        end else begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                    end;
                end;
                ...
                if el.FType=caeSemicolon then begin
                    ...
                    Result:=un.FExperssion.Compiler(ca,curclass,el,nil,@el2); if Result<>'' then Exit;
                    if el2=nil then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                    if el2.FType<>caeSemicolon then begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                    if el2.FNext=nil then begin Result:=el2.CErrorEXP(el2.FSme+el2.FLen); Exit; end;
                    el:=el2.FNext;
                ...
                if el.FType=caeSemicolon then begin
                    ...
                    Result:=CompilerR(ca,codeunit,funinclude,el,un,elendcomp,@el,unempty,{un2}un3,curclass); if Result<>'' then Exit;
                end;
                ...
                if (opBreak=nil) or (el.FNext=nil) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                if (el.FNext.FType<>caeSemicolon) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                ...
                if (opContinue=nil) or (el.FNext=nil) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                if (el.FNext.FType<>caeSemicolon) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                ...
                if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; end;
                if (el.FNext.FType<>caeStr) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;

                if not Assigned(funinclude) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                ...
                if (zint=2) or (zint=3) then begin
                    cainclude.Free;
                    Result:=el.FNext.CErrorEXP(el.FNext.FSme);
                    Exit;
                ...
                if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; end;
                if (el.FNext.FType<>caeName) or (not IsVarEXP(el.FNext.FStr)) or (el.FNext.FNext=nil) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                if (el.FNext.FNext.FType<>caeOpen1) or (el.FNext.FNext.FNext=nil) then begin Result:=el.FNext.FNext.CErrorEXP(el.FNext.FNext.FSme); Exit; end;
                ...
                while (el2<>nil) and (el2.FType=caeName) and (IsVarEXP(el2.FStr)) do begin
                    ...
                    el2:=el2.FNext; if el2=nil then begin Result:=ca.FLast.CErrorEXP(ca.FLast.FSme+ca.FLast.FLen); Exit; end;

                    if (oper<>'ref') and (el2.FType=caeAssume) then begin
                        if el2.FNext=nil then begin Result:=ca.FLast.CErrorEXP(ca.FLast.FSme+ca.FLast.FLen); Exit; end;
                        el2:=el2.FNext;
                        ...
                        else begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                    end;

                    if (el2=nil) or (el2.FType<>caeComma) then break;
                    el2:=el2.FNext;
                end;
                if (el2=nil) or (el2.FType<>caeClose1) then begin Result:=el.FNext.FNext.FNext.CErrorEXP(el.FNext.FNext.FNext.FSme); Exit; end;
                el:=el2.FNext;
                if el=nil then begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                if (el.FType<>caeOpen2) or (el.FNext=nil) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                ...
                if el2=nil then begin Result:=ca.FLast.CErrorEXP(ca.FLast.FSme+ca.FLast.FLen); Exit; end;
                if el2.FType<>caeClose2 then begin Result:=el2.CErrorEXP(el2.FSme); Exit; end;
                el:=el2.FNext;
                continue;
            end else if oper='class' then begin
                if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; end;
                if (el.FNext.FType<>caeName) or (not IsVarEXP(el.FNext.FStr)) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                if el.FNext.FNext=nil then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme+el.FNext.FLen); Exit; end;
                ...
                if el.FType=caeColon then begin
                    ...
                    while True do begin
                        ...
                        if (tvar2=nil) or (tvar2.VType<>vtFun) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                ...
                if (el.FType<>caeOpen2) then begin Result:=el.CErrorEXP(el.FSme); Exit; end;
                ...
                if el.FNext=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; end;
                if (el.FNext.FType<>caeOpen2) then begin Result:=el.FNext.CErrorEXP(el.FNext.FSme); Exit; end;
                ...
                if el=nil then begin Result:=el.CErrorEXP(el.FSme+el.FLen); Exit; end;
            ...
            dec(countopen);
            if (elcontinue<>nil) and (countopen=0) then begin
                elcontinue^:=Cardinal(el.FNext);
                Exit;
            end else if countopen=-1 then begin
                if elendcomp<>nil then elendcomp^:=Cardinal(el);
                Exit;
            end;
        end else begin
            Result:=el.CErrorEXP(el.FSme);
...
procedure TCodeEC.RunFunction(const name:WideString; cpr:TCodeProcessEC; cd:TCodeDebugEC; vars:TVarArrayEC);
var
	funvar:TVarEC;
    tcfun:TCodeEC;
    retvar:TVarEC;
begin
	...
	funvar:=LocalVar.GetVar(name);
    if funvar.VType<>vtFun then raise ExceptionExpressionEC.Create('Not fun:'+name);

	tcfun:=TCodeEC.Create;
    tcfun.FParent:=funvar.VFun;
	tcfun.CopyFromFast(funvar.VFun);

    retvar:=TVarEC.Create;

    if (vars<>nil) and (vars.Count>0) then retvar.SetRef(vars.Items[0]);
	try
    	tcfun.FLocalVar.GetVar('result').VRef:=retvar;
		if cd=nil then tcfun.Run(cpr)
		else tcfun.RunDebug(cpr,cd);
	except
		tcfun.Free;
