...
uses Windows,Classes,Graphics,Math,SysUtils,EC_Buf;
...
function Settings(path:WideString):WideString;
...
begin
    if FParent=nil then begin
        ...
        Result.x:={FParent.WorldPos.x}Result.x+FPos.x;
        Result.y:={FParent.WorldPos.y}Result.y+FPos.y;
    end;
end;
...
begin
    ...
    ca.Rectangle(rc.left,rc.top,rc.Right,rc.Bottom);
    ...
    DrawText(ca.Handle,PAnsiChar(AnsiString(text)),-1,crc,DT_TOP or DT_LEFT or DT_NOCLIP or DT_CALCRECT);
    ...
    if tar then begin
        ...
        DrawText(ca.Handle,PAnsiChar(AnsiString(text)),-1,crc,DT_LEFT or DT_TOP or DT_NOCLIP);
    end else begin
        crc:=Rect(rc.Left,y,rc.Right,1000000);
        DrawText(ca.Handle,PAnsiChar(AnsiString(text)),-1,crc,fl or DT_TOP or DT_NOCLIP);
    end;
